...
public class QuestionController {
    ...
    @Operation(summary = "分页获取题目列表", description = "分页获取题目列表（管理员）")
    ...
    @Operation(summary = "点赞题目", description = "用户对题目进行点赞或取消点赞")
    public BaseResponse<Boolean> thumbQuestion(@PathVariable @Min(value = 1, message = "ID必须大于0") long id, HttpServletRequest request) {
        boolean result = questionService.thumbQuestion(id, request);
        return ResultUtils.success(result);
    }
     ...
     * @return
     */
    @PostMapping("/favour/{id}")
    @Operation(summary = "收藏题目", description = "用户对题目进行收藏或取消收藏")
    public BaseResponse<Boolean> favourQuestion(@PathVariable @Min(value = 1, message = "ID必须大于0") long id, HttpServletRequest request) {
        boolean result = questionService.favourQuestion(id, request);
        return ResultUtils.success(result);
    }
}
